...
},getMediaArea:function(c,b){var a=null;
return this.callFunction("getMediaArea",[c,b],a)
...
if($pdk.bootloaderVersion!==null&&typeof($pdk.bootloaderVersion)==="object"){if("5"!=$pdk.bootloaderVersion.major||"6"!=$pdk.bootloaderVersion.minor||"12"!=$pdk.bootloaderVersion.revision){alert("Error: Bootloader Version and PDK-JS Version do not match.\n\nSomething is seriously wrong.")
...
}},onMediaLoadStart:function(f){var c=f.data;
...
var b=this.controller.getCurrentPlaylist();
var a=this.controller.getMainPlaylist();
this.setCurrentChapter();
if(c.baseClip.isAd){this.adIndex++
}if(b.playlistID!=a.playlistID){var d=this.differentPlaylists(b,this.lastCurrentPlaylist);
this.lastCurrentPlaylist=b;
if(d&&this.adCounts[this.currentChapter]>=1&&b.clips.length&&b.clips[0].baseClip.isAd){this.adCounts[this.currentChapter]+=b.clips.length-1
}}if(c.chapter&&c.chapter.chapters){this.setTimes(c.chapter.aggregateStartTime+c.offset,c.chapter.chapters.aggregateLength,this.adIndex,this.adCounts[this.currentChapter]);
...
}}else{if(c.baseClip.isAd){if(c.endTime>0){this.setTimes(c.startTime,c.endTime,this.adIndex,this.adCounts[this.currentChapter])
}else{this.setTimes(0,c.length,this.adIndex,this.adCounts[this.currentChapter])
}}else{if(c.baseClip.trueLength>0){this.setTimes(c.startTime+c.offset,c.baseClip.trueLength,0,0)
}else{this.setTimes(c.startTime+c.offset,this.releaseLength,0,0)
}}this.controlsManager.setChapterPoints([])
}if(c.baseClip.availableImageTracks&&typeof(c.baseClip.availableImageTracks)&&c.baseClip.availableImageTracks.length>0&&!c.baseClip.isAd){this.controlsManager.setAvailableImageTracks(c.baseClip.availableImageTracks)
...
return false
}else{if(b=="application/dash+xml"&&$pdk._browserCheck([{browser:"chrome",version:31,device:false}])){tpDebug("NOT stripping VTT; mime type is mpeg-dash and browser passes");
return false
...
if(a||this.isNoopClip(this.currentClip)||this.isProxyClip(this.currentClip)||this.playlistHasSSA){this.clipEnded=true;
...
this.video=null
}},videoClicked:function(d){if(this.currentClip){if(!tpIsIOS()&&!tpIsAndroid()){this.controller.pause(!this.getPauseState())
...
return false
}}},createMediaFileFromClip:function(a){return{url:a.URL,isAd:a.baseClip.isAd,mimeType:a.baseClip.type?a.baseClip.type:a.streamType,isExternal:this.playbackManager.isNoopClip(a)}
...
b.controller.dispatchEvent("OnContentAreaChanged",b.controller.getContentArea(true));
var c=b.controller.getMediaArea(false,true);
b.controller.dispatchEvent("OnMediaAreaChanged",c);
tpDebug(" MediaArea width/height is "+c.width+"/"+c.height);
...
}},getContentArea:function(i){try{var a=0;
var j=0;
if(!this.contentAreaSet&&this.currentClip){a=this.currentClip.baseClip.width;
j=this.currentClip.baseClip.height
}if(this.contentAreaSet||a===0||j===0){a=this.videoEngine.getVideoWidth();
j=this.videoEngine.getVideoHeight()
...
},getMediaArea:function(c,b){if(!this.standby){this.standby=document.getElementById(this.id+".standby")
}var a;
if(b||this._mediaArea===null||typeof(this._mediaArea)!=="object"){a={top:0,left:0,x:0,y:0,width:this.player.offsetWidth,height:this.player.offsetHeight}
...
}if(c){this.controller.dispatchEvent("OnMediaAreaChanged",a)
}return a
...
$pdk.classes.VideoEngineSubtitlesManager=$pdk.extend(function(){},{MIME_TYPE_VTT:"text/vtt",constructor:function(a,c){this.controller=a;
this.ve=c;
...
var b=this;
if(this.isIOS_compatible){a.addEventListener("OnGetSubtitleLanguage",function(d){b.onGetSubtitleLanguage(d)
});
a.addEventListener("OnReleaseStart",function(d){b.iosOnReleaseStart(d)
});
...
a.addEventListener("OnMediaStart",function(d){b.iosOnMediaStart(d)
})
}else{if(!$pdk.isIOS&&$pdk.isSafari){a.addEventListener("OnGetSubtitleLanguage",function(d){b.onGetSubtitleLanguage(d)
});
a.addEventListener("OnMediaLoadStart",function(d){b.safariOnMediaLoadStart(d)
})
}else{if(!$pdk.isIOS){a.addEventListener("OnGetSubtitleLanguage",function(d){b.onGetSubtitleLanguage(d)
});
a.addEventListener("OnMediaStart",function(d){b.onMediaStart(d)
})
...
for(c;
c<a;
c++){if(b[c].language==d&&b[c].MIMEType.toLowerCase()!=this.MIME_TYPE_VTT){return true
}}return false
...
}},showHideOnFirstClip:function(d){this.log("showHideOnFirstClip");
var a=d.baseClip.isAd;
if(this.currentVESubtitles&&this.currentVESubtitles.length>0){var e;
for(var b=0;
b<this.currentVESubtitles.length;
b++){var c=this.currentVESubtitles[b];
if(c.show){e=c;
break
}}if(e&&!a){this.log("showingSubtitles and is not an ad, so show them");
this.ve.showCaptions(e)
}else{this.log("showingSubtitles: "+(e?e.language:"no showingSubtitles object!")+", isAd:"+a);
this.ve.hideCaptions(false);
if(a){this.log("storing lastSelectedSubtitles");
this.lastSelectedSubtitles=e;
this.hideCaptionsWhenPlaying(true)
}else{this.log("NOT storing lastSelectedSubtitles")
...
if(!this.currentVESubtitles||this.currentVESubtitles.length<1){this.ve.hideCaptions(true)
}else{this.loadCaptions()
}},logTracks:function(b){var a=this
